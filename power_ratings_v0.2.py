# power_ratings_v0.2py
# last updated: 15 August 2020
# https://github.com/JohnBurant/COVID19-Model-Power-Ratings

# No command-line options, sorry. Just edit params and policy choices in the code.
...
d_policy = {'include_baselines': True,
            ...
            'include_models_without_full_measure_set': True,
...
l_outs = [] # Accumulates outputs in order to store in final .xslx file
...
for meas, vd in d_files_measures_cols.items():
    ...
    for k, v in d_week_pairs_str.items():
        for pr in v:
            ...
            df = pd.read_csv(eval_dir+fstring)
...
l_excluded_models = []
if not d_policy['include_baselines']: l_excluded_models += ['Baseline','COVIDhub-baseline']
...
max_weeks_expected = nweeks + 1 - np.maximum(i-32+nweeks,0)
...
# N.B. this is not currently written in the output file. But why aren't you using a notebook anyway?
...
# (Possibly more) complicated (than necessary) way to set up the index & output worksheet
